...
bool Mesh :: openOFF(const string fileName, unsigned int normWeight)
{
	...
	copy(V.begin(), V.end(), vertices.begin()) ;
	triangles.resize(T.size()) ;
	copy(T.begin(), T.end(), triangles.begin()) ;
	recomputeSmoothVertexNormals (normWeight);
	cout << "Finished load file: " << fileName << endl;
