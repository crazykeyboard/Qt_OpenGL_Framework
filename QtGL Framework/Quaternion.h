...
public:
	...
	Quaternion( float s, const Vec3Df& v );                 // initializes with specified real (s) and imaginary (v) components
	Quaternion( float s );                                  // initializes purely real quaternion with specified real (s) component
	Quaternion( const Vec3Df& v );                           // initializes purely imaginary quaternion with specified imaginary (v) component

	// ASSIGNMENT OPERATORS --------------------------------------------------
	const Quaternion& operator=( float s ); // assigns a purely real quaternion with real value s
	const Quaternion& operator=( const Vec3Df& v ); // assigns a purely real quaternion with imaginary value v
	...
	Vec3Df& im( void );                    // returns reference to imaginary part
	const Vec3Df& im( void ) const;              // returns const reference to imaginary part
...
protected:
	...
	Vec3Df v; // vector (imaginary) part
};
